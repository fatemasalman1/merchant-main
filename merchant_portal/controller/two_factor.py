...
def authenticate_for_2factor(user,method):
    ...
    cache_2fa_data(user, token, otp_secret, tmp_id,method)
    
    verification_obj,otp = get_verification_obj(user, token, otp_secret)
    # Save data in local
    ...
    if  production == 0:
        ...
        frappe.local.response["otp"] = otp


def cache_2fa_data(user, token, otp_secret, tmp_id,method):
...
def confirm_otp_token(Email, otp=None, tmp_id=None):
    """Confirm otp matches."""
    ...
    if not otp:
        if two_factor_is_enabled_for_(Email):
            return False
        return True
    ...
    tracker = get_login_attempt_tracker(Email)
    ...
    if hotp_token:
        if hotp.verify(otp, int(hotp_token)):
            ...
            frappe.local.response["message"] = frappe._("Incorrect Verification code")
            frappe.throw("Incorrect Verification code")
            

    totp = pyotp.TOTP(otp_secret)
    if totp.verify(otp):
        # show qr code only once
        if not get_default(Email + "_otplogin"):
            set_default(Email + "_otplogin", 1)
            delete_qrimage(Email)
        tracker.add_success_attempt()
        return True
    else:
        tracker.add_failure_attempt()
...
def get_verification_obj(user, token, otp_secret):
    ...
    if verification_method == "SMS":
        ...
        if not get_default(user + "_otplogin"):
            verification_obj,otp = process_2fa_for_email(user, token, otp_secret, otp_issuer, method="OTP App")
        else:
            verification_obj = process_2fa_for_otp_app(user, otp_secret, otp_issuer)
    elif verification_method == "Email":
        verification_obj,otp = process_2fa_for_email(user, token, otp_secret, otp_issuer)
    return verification_obj,otp
...
def process_2fa_for_email(user, token, otp_secret, otp_issuer, method="Email"):
    ...
    status,otp = send_token_via_email(user, token, otp_secret, otp_issuer, subject=subject, message=message)
    verification_obj = {
        "token_delivery": status,
...
def get_email_subject_for_2fa(kwargs_dict):
    ...
    subject_template = _("Login Verification Code from Baseeta")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
    ...
    lead=frappe.db.get_value("Merchant Lead",{"email":user_email},"name")
  
    if lead:
        frappe.db.set_value("Merchant Lead",lead,"otp",otp)
